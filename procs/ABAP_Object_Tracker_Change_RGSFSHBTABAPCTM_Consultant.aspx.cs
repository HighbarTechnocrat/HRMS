...
public partial class ABAP_Object_Tracker_Change_RGSFSHBTABAPCTM_Consultant : System.Web.UI.Page
{
    ...
    public void DDLProjectLocation_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            if(DDLProjectLocation.SelectedValue == "0")
            {
                ...
                DDLFSFunctionalConsultant.ClearSelection();
            }
            lblmessage.Text = "";
            ...
            spars[5] = new SqlParameter("@Module", SqlDbType.VarChar);
            spars[5].Value = DDLModule.SelectedValue;
            ...
            if (DDLStage.SelectedValue != "")
            {
                ...
                if (DS != null && DS.Tables.Count > 0 && DS.Tables[1].Rows.Count > 0)
                {
                    ...
                    if (DDLStage.SelectedValue == "RGS")
                    {
                        ...
                        var filteredRows = originalTable.AsEnumerable().Where(row => !row.IsNull("RGSDetailsId"));
                        DataTable RGSfilteredTable;
                        if (filteredRows.Any())
                        {
                            RGSfilteredTable = filteredRows.CopyToDataTable();
                        }
                        else
                        {
                            RGSfilteredTable = originalTable.Clone();
                    ...
                    if (DDLStage.SelectedValue == "FS")
                    {
                       ...
                       //DataTable FSfilteredTable = originalTable.AsEnumerable().Where(row => !row.IsNull("FSDetailsId")).CopyToDataTable();
                        ...
                        var filteredRows = originalTable.AsEnumerable().Where(row => !row.IsNull("FSDetailsId"));
                        DataTable FSfilteredTable;
                        if (filteredRows.Any())
                        {
                            FSfilteredTable = filteredRows.CopyToDataTable();
                        }
                        else
                        {
                            FSfilteredTable = originalTable.Clone();
                    ...
                    if (DDLStage.SelectedValue == "ABAP")
                    {
                        ...
                        var filteredRows = originalTable.AsEnumerable().Where(row => !row.IsNull("ABAPDetailsId"));
                        DataTable ABAPfilteredTable;
                        if (filteredRows.Any())
                        {
                            ABAPfilteredTable = filteredRows.CopyToDataTable();
                        }
                        else
                        {
                            ABAPfilteredTable = originalTable.Clone();
                            lblmessage.Text = "No records found.";
                            gvABAPDevDetails.Visible = false;
                            spnabap.Visible = false;
                        }
                    ...
                    if (DDLStage.SelectedValue == "HBT Testing")
                    {
                        ...
                        var filteredRows = originalTable.AsEnumerable().Where(row => !row.IsNull("HBTDetailsId"));
                        DataTable HBTfilteredTable;
                        if (filteredRows.Any())
                        {
                            HBTfilteredTable = filteredRows.CopyToDataTable();
                        }
                        else
                        {
                            HBTfilteredTable = originalTable.Clone();
                    ...
                    if (DDLStage.SelectedValue == "CTM Testing")
                    {
                        ...
                        var filteredRows = originalTable.AsEnumerable().Where(row => !row.IsNull("CTMDetailsId"));
                        DataTable CTMfilteredTable;
                        if (filteredRows.Any())
                        {
                            CTMfilteredTable = filteredRows.CopyToDataTable();
                        }
                        else
                        {
                            CTMfilteredTable = originalTable.Clone();
                            lblmessage.Text = "No records found.";
                            gvCTMTestDetails.Visible = false;
                            spnctmtest.Visible = false;
                        }
            ...
            else
            {
                ...
                DDLModule.DataSource = DS.Tables[2];
    ...
    protected void updatefs_btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (Convert.ToString(DDLStage.SelectedValue).Trim() == "0" || Convert.ToString(DDLStage.SelectedValue).Trim() == "")
        {
            lblmessage.Text = "Please select the stage.";
            return;
        }
        if (Convert.ToString(DDLModule.SelectedValue).Trim() == "0" || Convert.ToString(DDLModule.SelectedValue).Trim() == "")
        {
