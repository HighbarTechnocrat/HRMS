...
public partial class procs_VSCB_ApprovedPOWOList : System.Web.UI.Page
{
        ...
        try
        {
            ...
            else
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    {
                        if(Convert.ToString(Request.QueryString[0]).Trim()=="y")
                        {
                            lblmessage.Text = "PO/ WO details deleted successfully.";
                        }
                    }
    ...
    protected void lnkLeaveDetails_Click(object sender, EventArgs e)
    {
        ...
        string StrStatus = Convert.ToString(gvMngTravelRqstList.DataKeys[row.RowIndex].Values[1]).Trim();


        //#region   Delete Temp Milestone
        //DataTable dsInvoice = new DataTable();
        //SqlParameter[] spars = new SqlParameter[2];

        //spars[0] = new SqlParameter("@stype", SqlDbType.VarChar);
        //spars[0].Value = "Delete_temp_Milestone";

        //spars[1] = new SqlParameter("@empcode", SqlDbType.VarChar);
        //spars[1].Value = Convert.ToString(hdnEmpCode.Value).Trim();

        //dsInvoice = spm.getDataTableList_VSCB(spars, "SP_VSCB_GETALL_DETAILS");

        //#endregion


        Response.Redirect("VSCB_POWODelete.aspx?poid=" + hdnPOWOID.Value + "&mngexp=1" + "&ST=" + StrStatus.ToString().Trim() + "");
    }
    ...
    private void getMngTravelReqstList()
    {
        try
        {
            ...
            spars[0] = new SqlParameter("@stype", SqlDbType.VarChar);
            spars[0].Value = "get_msgApprovedMilestoneList";
            ...
            spars[5] = new SqlParameter("@Project_Dept_Name", SqlDbType.BigInt);
            if (Convert.ToString(lstDepartment.SelectedValue).Trim() != "0")
                spars[5].Value = Convert.ToDouble(lstDepartment.SelectedValue);
            else
                spars[5].Value = DBNull.Value;

            spars[6] = new SqlParameter("@costcenter", SqlDbType.VarChar);
            if (Convert.ToString(lstCostcenter.SelectedValue).Trim() != "0")
                spars[6].Value = Convert.ToString(lstCostcenter.SelectedValue).Trim();
            else
                spars[6].Value = DBNull.Value;

            spars[7] = new SqlParameter("@POTypeID", SqlDbType.Int);
            if (Convert.ToString(lstPoWoType.SelectedValue).Trim() != "0")
                spars[7].Value = Convert.ToInt32(lstPoWoType.SelectedValue);
            else
    ...
    private void get_myPOWOMilestone_DropdownList()
    {
        ...
        spars[0] = new SqlParameter("@stype", SqlDbType.VarChar);
        spars[0].Value = "get_ApprovedListpowo_Milestone_dropdown_List";
        ...
        {
            lstDepartment.DataSource = dsList.Tables[3];
            lstDepartment.DataTextField = "Project_Dept_Name";
            lstDepartment.DataValueField = "Prj_Dept_id";
        ...
        {
            lstPoWoType.DataSource = dsList.Tables[5];
            lstPoWoType.DataTextField = "POType";
            lstPoWoType.DataValueField = "POTypeID";
            lstPoWoType.DataBind();
        }
        lstPoWoType.Items.Insert(0, new ListItem("Select PO/WO Type", "0"));
    ...
    protected void localtrvl_btnSave_Click(object sender, EventArgs e)
    {
            ...
            //    spars[1].Value = lstPOWONo.SelectedValue;
            ...
            //}
            //{
            //    lblmessage.Text = "Select PO / Wo No. ";
            //}
        }
}