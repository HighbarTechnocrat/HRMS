                    ...
                    if (worksheet.RowsUsed().Count() > 1)
                    {
                        ...
                        for (int row = 3; row <= worksheet.LastRowUsed().RowNumber(); row++)
                        {
                            ...
                            for (int col = 1; col <= columnCount; col++)
                            {
                                ...
                                var cell = headerRow1.Cell(col);
                                if (cell.IsMerged())
                                {
                                    var mergedRange = cell.MergedRange();
                                     header1 = mergedRange.FirstCell().GetString(); // or .Value.ToString()
                                }
                                else
                                {
                                     header1 = cell.GetString(); // or .Value.ToString()
                                }
                                ...
                                {
                                    int intResult;
                                    if ((col == 1 || col == 7 || col == 18 || col == 29 || col == 30) && (!int.TryParse(cellValue != null ? cellValue.ToString() : "", out intResult)))
                                    {
                                        lbl_Upload_Error.Text = "Invalid entry in row " + row + ", column '" + columnHeader + "' : value must contain integer values.";
                                        return;
                                    }
                                    if ((col == 11 || col == 12 || col == 13 || col == 14 || col == 15 || col == 16 || col == 19 || col == 20 || col == 21 || col == 22 || col == 23 || col == 24 || col == 25|| col == 26 ) && !Regex.IsMatch(cellValue, @"^(0[1-9]|[12][0-9]|3[01])\.(0[1-9]|1[0-2])\.\d{4}$"))
                                    {
                                        lbl_Upload_Error.Text = "Invalid entry in row " + row + ", column '" + columnHeader + "' : value must be in dd.mm.yyyy format.";
                                        return;
                                    }


                                    if ((col == 3 || col == 5) && Regex.IsMatch(cellValue, @"\d"))
                                    {
                                        lbl_Upload_Error.Text = "Invalid entry in row " + row + ", column '" + columnHeader + "' : value must contain letters only.";
                                        return;
                                    }
