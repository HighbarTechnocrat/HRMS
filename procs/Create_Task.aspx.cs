            ...
            if (getDs != null)
            {
                if (getDs.Tables.Count > 0)
                {
                    ...
                    if (getTaskRefDetails.Rows.Count > 0)
                    {
                        ...
                        spm.Task_Monitoring_All_Attendee(getTaskDetails, StrExecuter, Meeting_Discussion_Title, Meeting_Discussion_Date, MeetingType, Task_Reference_ID, appendIntimation);
                        var link = "http://localhost/hrms/login.aspx?ReturnUrl=procs/InboxExecuter.aspx";
                        if (ddl_Meeting_Type.SelectedItem.Value != "4")
                        {
                            // Comment by harshad for seperate mail go to Executer only 
