...
public partial class Cancel_Task : System.Web.UI.Page
{
    ...
    private void BindData(string qtype)
    {
        try
        {
            ...
            var getResult = spm.getTaskMonitoringDDL(empCode, getVal, "GetAllDDL_Cancel");
            if (getResult != null)
            {
                if (getResult.Tables.Count > 0)
                {
                    if (qtype == "All")
                    {
                        ...
                        {
                            BindGridTaskIntimation(getIntimationList);
                        }
                    }
    ...
    private void BindGridTaskIntimation(DataTable dataTable)
    {
        try
        {
            GridViewTaskIntimation.DataSource = null;
            GridViewTaskIntimation.DataBind();
            if (dataTable.Rows.Count > 0)
            {
                GridViewTaskIntimation.DataSource = dataTable;
                GridViewTaskIntimation.DataBind();
            }
            else
            {
    ...
    private void ClearTask()
    {
        try
        {
            ...
            hdnTaskID.Value = "0";
            txt_ActionRemarks.Text = "0";
            IsShowActionRemark.Visible = false;
            IsShowActionRemark1.Visible = false;
        }
        catch (Exception)
        {
            ...
            foreach (GridViewRow row in dg_OldTaskDetails.Rows)
            {
                ...
                if (ddl1.Checked == true)
                {
                    ...
                    if (getRefAdd == 0)
                    {
                        ...
                        var getStatus = spm.InsertUpdateTaskDetails(0, 0, "AddTaskReference", empCode, empCode, getMettingDiscussionTitle, getMettingDiscussionDate, Convert.ToInt32(getMettingDiscussionType), false, getOldTaskRef, "", "", "", "", "", "", false, 0, false, 0, false,0,"",0);
                    ...
                    var getStatusTask = spm.InsertUpdateTaskDetails(getlblTask_id, getRefAdd, "AddTaskRef", empCode, empCode, "", "", 0, false, getOldTaskRef, "", "", "", "", "", "", false, 0, false, 0, false,0, "",0);
    ...
    protected void txt_Due_Date_TextChanged(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (tempF < tempT)
            {
                Label3.Text = "Due date cannot be less than today date";
                txt_Due_Date.Text = "";
                return;
            }
        }
    ...
    protected void lnk_Task_Create_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (btn.Text == "Cancel")
            {
                ...
                var getStatus = spm.UpdateDueDateChangeStatus(Convert.ToDouble(id), "UpdateCancelStatus", getCreatedBy, Convert.ToString(txt_ActionRemarks.Text).Trim(), false, getExecuter);
                if (getStatus== true)
                {
                    var link = "https://ess.highbartech.com/hrms/login.aspx?ReturnUrl=procs/InboxExecuter.aspx";
                    
                    var getTaskEmailDetails = spm.getTaskMonitoringReport("getTaskDetailsByExecuterClose", getExecuter, Convert.ToDouble(hdnTaskRefID.Value), Convert.ToDouble(hdnTaskID.Value));
                    if (getTaskEmailDetails != null)
                    {
                        if (getTaskEmailDetails.Tables.Count > 0)
                        {
                            ...
                            var getTaskIntimationDetails = getTaskEmailDetails.Tables[2];
                            ...
                            if (getTaskDetails1.Rows.Count > 0)
                            {
                                ...
                                var CreatedBy = Convert.ToString(getTaskDetails1.Rows[0]["CreatedBy"]);

                                var TaskIntimationEmail =  Convert.ToString(getTaskIntimationDetails.Rows[0]["CCEmailAddress"]);
                                if (TaskIntimationEmail != "")
                                {
                                    SupervisorEmail = SupervisorEmail + ";" + TaskIntimationEmail;
                                }

                                // SupervisorEmail = "";
                                link = "https://ess.highbartech.com/hrms/login.aspx?ReturnUrl=procs/TaskExecuter_Edit.aspx?Task_Id=" + hdnTaskID.Value + "&TaskRefId=" + hdnTaskRefID.Value + "&flag=NA";
                                spm.Task_Monitoring_Executer_Cancel(getTaskDetails1, ExecuterEmail, Meeting_Discussion_Title, Meeting_Discussion_Date, MeetingType, Task_Reference_ID, link, SupervisorEmail, CreatedBy);
                            }
                        }
                        //Sub Task
                        var getTaskEmailDetailsSub = spm.getTaskMonitoringReport("getSubTaskDetailsByExeCancel", getExecuter, Convert.ToDouble(hdnTaskRefID.Value), Convert.ToDouble(hdnTaskID.Value));
                        if (getTaskEmailDetailsSub != null)
                        {
                            if (getTaskEmailDetailsSub.Tables.Count > 0)
                            {
                                var getTaskDetails1          = getTaskEmailDetailsSub.Tables[0];
                                var getTaskRefDetails        = getTaskEmailDetailsSub.Tables[1];
                                var getTaskIntimationDetails = getTaskEmailDetails.Tables[2];
    ...
    protected void lnk_TLI_Edit_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            ...
            if (fId != "0")
            {
                ...
                var getdsTables = spm.getTaskDetailsById(getCreatedBy, Convert.ToDouble(TaskRefID), Convert.ToDouble(fId), "getTaskDetailsById_Cancel");
                ...
                if (getdsTables.Tables.Count > 0)
                {
                    ...
                    var getcomp_code = Convert.ToString(getds.Rows[0]["comp_code"]);
                    ...
                    if (Convert.ToBoolean(getds.Rows[0]["For_Information_Only"]) == false)
                    {
                        ...
                        DDLProjectLocation.Items.FindByValue(getcomp_code).Selected = true;
                    ...
                    if (Convert.ToString(getds.Rows[0]["StatusName"]) != "Closed" && Convert.ToString(getds.Rows[0]["StatusName"]) != "Cancelled")//
                    {
                        lnk_Task_Update.Visible = false;
                        lnk_Task_Cancel.Visible = false;
                        lnk_Task_Create.Visible = true;
                        IsShowActionRemark.Visible = true;                      
                        IsShowActionRemark1.Visible = true;                      
                    }
                    else
                    {
                        lnk_Task_Update.Visible = false;
    ...
    protected void lnk_Final_Submit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            var getMettingDiscussionType = Convert.ToString(ddl_Meeting_Type.SelectedValue).ToString();
            var getCreatedBy = Convert.ToString(Session["Empcode"]).Trim();
            var getStatus = spm.InsertUpdateTaskDetails(Convert.ToInt32(getRefId), getRefId, "UpdateTaskRef", getCreatedBy, getCreatedBy, getMettingDiscussionTitle, getMettingDiscussionDate, Convert.ToInt32(getMettingDiscussionType), false, getOldTaskRef, "", "", "", "", "", "", false, 0, false, 0, false,0,"",0);
            //
                    ...
                    if (getTable != null)
                    {
                        if (getTable.Rows.Count > 0)
                        {
                            foreach (DataRow item in getTable.Rows)
                            {
                                ...
                                var getStatus1 = spm.InsertUpdateTaskDetails(id, getTaskRefId, "UpdateNewTaskDetails", getCreatedBy, getCreatedBy, getMettingDiscussionTitle, getMettingDiscussionDate, Convert.ToInt32(getMettingDiscussionType), getFor_Info, getOldTaskRef, getTaskSupervisor, getTaskExecutor, getTaskDescripation, getTaskRemark, getDueDate, getTask_ID, getReminder_Due_Date, getReminderDay, getEscalation_Due_Date, getEscalationDay, getEscalation_Repeate, tempTaskRefId, getProjectLocationCode,0);
            ...
            if (getMettingOldref != "0")
            {
                ...
                foreach (GridViewRow row in dg_OldTaskDetails.Rows)
                {
                    ...
                    if (ddl1.Checked == true)
                    {
                        ...
                        Label lblTask_id = row.FindControl("lblTask_id") as Label;
                        var getlblTask_id = Convert.ToInt32(lblTask_id.Text.Trim());
                        var getStatusTask = spm.InsertUpdateTaskDetails(getlblTask_id, getRefId, "AddTaskRef", empCode, empCode, "", "", 0, false, getOldTaskRef, "", "", "", "", "", "", false, 0, false, 0, false,0,"",0);
                     }
                }
            }
            //End Email
