...
public partial class Update_Task : System.Web.UI.Page
{
    ...
    private void DeleteTempAttendee()
    {
        try
        {
            ...
            spm.InsertUpdateTempAttendee(0, Convert.ToInt32(hdnTaskRefID.Value), "DELETETempAttendee", getEmpCode, getEmpCode, false);
        }
        catch (Exception ex)
        {
                ...
                if (getDs != null)
                {
                    if (getDs.Tables.Count > 0)
                    {
                        ...
                        if (getTaskRefDetails.Rows.Count > 0)
                        {
                            ...
                            var getLatestTaskCreateTemp =  getDs.Tables[5];

                            if (getLatestTaskCreateTemp.Rows.Count > 0) // Temp Excuter and Supervisor and other
                            {
                                 getAllAttendee = getDs.Tables[6];
                                 getAllTaskIntimation = getDs.Tables[7];
                                 getAllExecuter = getDs.Tables[8];
                            }
                            ...
                            if (appendIntimation == "")
                            {
                                appendIntimation = appendAttendee + ";" + Convert.ToString(getAllTaskIntimation.Rows[0]["CCEmailAddress"]);
                            }
                            ...
                            foreach (DataRow item in getAllExecuter.Rows)
                            {
                                if (StrExecuter == "")
                                {
                                    StrExecuter = Convert.ToString(item["Emp_Emailaddress"]);
                                }
                                else
                                {
                                    StrExecuter = StrExecuter + ";" + Convert.ToString(item["Emp_Emailaddress"]);
                                }
                            }
                            ...
                            if (getLatestTaskCreateTemp.Rows.Count > 0)
                            {
                                spm.Task_Monitoring_All_Attendee(getLatestTaskCreateTemp, StrExecuter, Meeting_Discussion_Title, Meeting_Discussion_Date, MeetingType, Task_Reference_ID, appendIntimation);
                            }

                            #region Comment by harshad for seperate mail go to Executer only 
