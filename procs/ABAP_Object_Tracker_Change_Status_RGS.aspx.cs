...
public partial class ABAP_Object_Tracker_Change_Status_RGS : System.Web.UI.Page
{
    ...
    {
        gvRGSDetails.Columns[0].Visible = false;

        Response.Clear();
        Response.Buffer = true;
        Response.AddHeader("content-disposition", "attachment;filename=RGSDetailPlanExport.xls");
        Response.Charset = "";
        ...
        using (StringWriter sw = new StringWriter())
        {
            using (HtmlTextWriter hw = new HtmlTextWriter(sw))
            {
                ...
                gvRGSDetails.RenderControl(hw);
                Response.Output.Write(sw.ToString());
                Response.Flush();
                Response.End();
            }
        }
        gvRGSDetails.Columns[0].Visible = true;
    }
