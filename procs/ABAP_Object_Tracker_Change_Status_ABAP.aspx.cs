...
public partial class ABAP_Object_Tracker_Change_Status_ABAP : System.Web.UI.Page
{
    ...
    public void get_ABAP_Detail()
    {
        ...
        if (dsABAPObjectPlanSubmitted != null && dsABAPObjectPlanSubmitted.Tables[0].Rows.Count > 0)
        {
            ...
            if(!string.IsNullOrEmpty(Convert.ToString(dsABAPObjectPlanSubmitted.Tables[0].Rows[0]["ReusableClientName"])))
            {
                li_ReusableClientName.Visible = true;
                txt_ReusableClientName.Text = Convert.ToString(dsABAPObjectPlanSubmitted.Tables[0].Rows[0]["ReusableClientName"]);
            }
            else
            {
                li_ReusableClientName.Visible = false;
            }

            if (!string.IsNullOrEmpty(Convert.ToString(dsABAPObjectPlanSubmitted.Tables[0].Rows[0]["ResuableStatusName"])))
            {
                li_ReusableStatus.Visible = true;
                txt_ReusableStatus.Text = Convert.ToString(dsABAPObjectPlanSubmitted.Tables[0].Rows[0]["ResuableStatusName"]);
            }
            else
            {
                li_ReusableStatus.Visible = false;
            }

            if (!string.IsNullOrEmpty(Convert.ToString(dsABAPObjectPlanSubmitted.Tables[0].Rows[0]["ReusableAdditonalEffort"])))
            {
                li_ReusableAdditonalEffort.Visible = true;
                txt_ReusableAdditonalEffort.Text = Convert.ToString(dsABAPObjectPlanSubmitted.Tables[0].Rows[0]["ReusableAdditonalEffort"]);
            }
            else
            {
                li_ReusableAdditonalEffort.Visible = false;
            }

            if (!string.IsNullOrEmpty(Convert.ToString(dsABAPObjectPlanSubmitted.Tables[0].Rows[0]["ReusablePercent"])))
            {
                li_ReusablePercent.Visible = true;
                txt_ReusablePercent.Text = Convert.ToString(dsABAPObjectPlanSubmitted.Tables[0].Rows[0]["ReusablePercent"]);
            }
            else
            {
                li_ReusablePercent.Visible = false;
            }
            ...
            if (!string.IsNullOrEmpty(Convert.ToString(dsABAPObjectPlanSubmitted.Tables[0].Rows[0]["ReusableDetailsRemarks"])))
            {
                li_ReusableDetailsRemarks.Visible = true;
                txt_ReusableDetailsRemarks.Text = Convert.ToString(dsABAPObjectPlanSubmitted.Tables[0].Rows[0]["ReusableDetailsRemarks"]);
            }
            else
            {
                li_ReusableDetailsRemarks.Visible = false;
            }

            if (!string.IsNullOrEmpty(Convert.ToString(dsABAPObjectPlanSubmitted.Tables[0].Rows[0]["ReusableDetailsRemarks"])))
            {
                li_Custom_Tcode.Visible = true;
                txt_Custom_Tcode.Text = Convert.ToString(dsABAPObjectPlanSubmitted.Tables[0].Rows[0]["Custom_Tcode"]);
            }
            else
            {
                li_Custom_Tcode.Visible = false;
            }
    ...
    public void get_ABAP_Object_Submitted_Plan_FSDetails()
    {
        try
        {
            ...
            spars[0] = new SqlParameter("@qtype", SqlDbType.VarChar);
            spars[0].Value = "GetRGSFSABAPHBTTestingDetailsByProjectLocation_ABAPDev";
    ...
    protected void LinkBtnSavePopup_Click(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrWhiteSpace(Txt_RevisedActualPreparationStart.Text) && !string.IsNullOrWhiteSpace(txt_RevisedActualPreparationFinish.Text))
        {
            ...
            if (startDate > finishDate)
            {
                ...
                PnlIrSheet.Visible = true;
                this.ModalPopupExtenderIRSheet.Show();
                lblmessagesub.Text = "Revised Start Date must be earlier than or equal to Revised Finish Date";
                return;
            }
            ...
            if (finishDate.Date < DateTime.Now.Date)
            {
                if (txt_RevisedActualPreparationFinish.Enabled)
                {
                    ...
                    PnlIrSheet.Visible = true;
                    this.ModalPopupExtenderIRSheet.Show();
                    lblmessagesub.Text = "Revised Finish Date must must not be less than today's date.";
                    return;
                }
            }
        }
        ...
        if (!string.IsNullOrWhiteSpace(Txt_RevisedActualSubmission.Text))
        {
            ...
            if (startDate.Date > DateTime.Now.Date)
            {
                ...
                PnlIrSheet.Visible = true;
                this.ModalPopupExtenderIRSheet.Show();
                lblmessagesub.Text = "Actual Start Date cannot be future date.";
                return;
            }
        }
    ...
    {
        gvABAPDevDetails.Columns[0].Visible = false;

        Response.Clear();
        Response.Buffer = true;
        Response.AddHeader("content-disposition", "attachment;filename=ABAPDevelopmentDetailPlanExport.xls");
        Response.Charset = "";
        ...
        using (StringWriter sw = new StringWriter())
        {
            using (HtmlTextWriter hw = new HtmlTextWriter(sw))
            {
                ...
                gvABAPDevDetails.RenderControl(hw);
                Response.Output.Write(sw.ToString());
                Response.Flush();
                Response.End();
            }
        }
        gvABAPDevDetails.Columns[0].Visible = true;
    }
