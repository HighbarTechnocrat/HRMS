...
public partial class AppreciationLetter_ViewRedmption
 : System.Web.UI.Page
{
    ...
    public void PopulateEmployeeData()
    {
        try
        {
            ...
            SqlParameter[] spars = new SqlParameter[2];
            spars[0] = new SqlParameter("@qtype", SqlDbType.VarChar);
            spars[0].Value = "employee_Redmption";
            ...
            if (dtEmp.Tables[0].Rows.Count > 0)
            {
                ...
                if (dtEmp.Tables[0].Rows[0]["remarkby_admin"] != DBNull.Value)
                {
                    txtremark_admin.Text = dtEmp.Tables[0].Rows[0]["remarkby_admin"].ToString();
                    txtremark_admin.Visible = true;
                    RemarksbyAdmin.Visible = true;
                }
                else
                {
                    txtremark_admin.Visible = false;
                    RemarksbyAdmin.Visible = false;
                }

                if (dtEmp.Tables[0].Rows[0]["admin_Redeem_Point"] != DBNull.Value)
                {
                    txtAdmin_Point.Text = dtEmp.Tables[0].Rows[0]["admin_Redeem_Point"].ToString();
                    txtAdmin_Point.Visible = true;
                    RequistRedeemPoint.Visible = true;
                }
                else
                {
                    txtAdmin_Point.Visible = false;
                    RequistRedeemPoint.Visible = false;
                }
