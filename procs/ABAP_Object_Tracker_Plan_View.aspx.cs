...
public partial class ABAP_Object_Tracker_Plan_View : System.Web.UI.Page
{
    ...
    protected void lnkABAPPlanDetails_Click(object sender, EventArgs e)
    {
        ...
        hdnABAPODUploadId.Value = Convert.ToString(gvDetailPlan.DataKeys[row.RowIndex].Values[0]).Trim();
        hdnABAPODIdId.Value = Convert.ToString(Request.QueryString["ABAPODId"]).Trim(); 
        Response.Redirect("ABAP_Object_Tracker_Create_List.aspx?Update=" + hdnABAPODUploadId.Value + "&ABAPODId=" + hdnABAPODIdId.Value);
    ...
    protected void GridView_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            ImageButton lnkEdit = (ImageButton)e.Row.FindControl("lnkABAPPlanDetails");

            if (lnkEdit != null)
            {
                object createdByObj = DataBinder.Eval(e.Row.DataItem, "Created_By");

                if (createdByObj != null)
                {
                    string createdBy = createdByObj.ToString().Trim();
                    string strempcode = Session["Empcode"] != null ? Session["Empcode"].ToString().Trim() : "";

                    if (createdBy == strempcode)
                    {
    ...
    public void get_ABAP_Object_Submitted_Plan_Details_View()
    {
        ...
        if (dsABAPObjectPlanSubmitted != null && dsABAPObjectPlanSubmitted.Tables[0].Rows.Count > 0)
        {
            ...
            if (dsABAPObjectPlanSubmitted.Tables[0].Rows[0]["UserRole"].ToString() == "PM" || dsABAPObjectPlanSubmitted.Tables[0].Rows[0]["UserRole"].ToString() == "PRM")
            {
                ...
                string isCreatorLoginSame = dsABAPObjectPlanSubmitted.Tables[0].Rows[0]["IsObjectCreate"].ToString().Trim();

                foreach (GridViewRow row in gvDetailPlan.Rows)
                {
                    if (isCreatorLoginSame == "FALSE")
                    {
                        gvDetailPlan.Columns[0].Visible = false;
                    }
                }

                btnTra_Details.PostBackUrl = "~/procs/ABAP_Object_Tracker_All_List.aspx";
            }
            else if (dsABAPObjectPlanSubmitted.Tables[0].Rows[0]["UserRole"].ToString() == "Consultant")
            {
                ...
                btnTra_Details.PostBackUrl = "~/procs/ABAP_Object_Tracker_ApprovedList.aspx";
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
        ...
        gvDetailPlan.Columns[0].Visible = true;
