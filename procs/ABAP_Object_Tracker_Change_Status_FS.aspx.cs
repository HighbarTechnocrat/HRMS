...
public partial class ABAP_Object_Tracker_Change_Status_FS : System.Web.UI.Page
{
    ...
    {
        gvFSDetails.Columns[0].Visible = false;

        Response.Clear();
        Response.Buffer = true;
        Response.AddHeader("content-disposition", "attachment;filename=FSDetailPlanExport.xls");
        Response.Charset = "";
        ...
        using (StringWriter sw = new StringWriter())
        {
            using (HtmlTextWriter hw = new HtmlTextWriter(sw))
            {
                ...
                gvFSDetails.RenderControl(hw);
                Response.Output.Write(sw.ToString());
                Response.Flush();
                Response.End();
            }
        }
        gvFSDetails.Columns[0].Visible = true;
    }
