					...
					if (buttonName == 'title') {
						...
						if (buttonClick) {
							...
							if (customText) {
								...
								if(themeIcon=='circle-triangle-w')
								{
									html = "<span class='ui-icon ui-icon-" + themeIcon + "'><i class='fa fa-arrow-left'></i></span>";
								}
								if(themeIcon=='circle-triangle-e')
								{
									html = "<span class='ui-icon ui-icon-" + themeIcon + "'><i class='fa fa-arrow-right'></i></span>";
								}
							}
...
function BasicView(element, calendar, viewName) {
	...
	function buildBodyHTML() {
		...
		for (row=0; row<rowCnt; row++) {
			...
			html += "<tr class='fc-week fc-custom-week'>";
...
function DayEventRenderer() {
	...
	function buildHTMLForSegment(segment) {
		...
		html +=
			"<span class='fc-event-title'><i class='fa fa-circle'></i><span class='lbltext'> " +
			htmlEscape(event.title || '') +
			"</span></span>" +
			"</div>";
		...
		//if (html.includes("left:1px;") == true) {
		//    html = "";
	    //}
		if (segment.left == 1) {
		    html = "<div></div>";
