...
public partial class ABAP_Object_Tracker_Change_Status_UATSignoff : System.Web.UI.Page
{
    ...
    protected void LinkBtnSavePopup_Click(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrWhiteSpace(txt_RevisedDate.Text))
        {
            if (RevisedDatedt < PlannedDatedt)
            {
                DivIDPopup.Visible = true;
                PnlIrSheet.Visible = true;
                this.ModalPopupExtenderIRSheet.Show();
                lblmessagesub.Text = "Revised Date must be greater than Planned Date.";
                return;
            }
        }
        ...
        {
            if (ActualDatedt < PlannedDatedt)
            {
                DivIDPopup.Visible = true;
                PnlIrSheet.Visible = true;
                this.ModalPopupExtenderIRSheet.Show();
                lblmessagesub.Text = "Actual Date must be greater than or equal to Planned Date.";
                return;
            }
        }

        if (!string.IsNullOrWhiteSpace(Txt_Final_Script_Sub_Date.Text))
        {
            if (FinalSubmitdt < PlannedDatedt)
            {
                DivIDPopup.Visible = true;
                PnlIrSheet.Visible = true;
                this.ModalPopupExtenderIRSheet.Show();
                lblmessagesub.Text = "Final Date must be greater than or equal to Planned Date.";
                return;
            }
        }
