...
public partial class thankyoucard_report : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
           ...
           // thankyoucard_access();
            ...
            if (Convert.ToString(hflEmpCode.Value) == "00631294" || Convert.ToString(hflEmpCode.Value) == "00630814" || Convert.ToString(hflEmpCode.Value) == "00003851" || Convert.ToString(hflEmpCode.Value) == "00630967" || Convert.ToString(hflEmpCode.Value) == "00631230" || Convert.ToString(hflEmpCode.Value) == "00631087" || Convert.ToString(hflEmpCode.Value) == "00631230" || Convert.ToString(hflEmpCode.Value) == "00630008" || Convert.ToString(hflEmpCode.Value) == "00630674" || Convert.ToString(hflEmpCode.Value) == "00631300" || Convert.ToString(hflEmpCode.Value) == "00631257" || Convert.ToString(hflEmpCode.Value) == "00631276"
         || Convert.ToString(hflEmpCode.Value) == "00631710" || Convert.ToString(hflEmpCode.Value) == "00631609" || Convert.ToString(hflEmpCode.Value) == "00631509" || Convert.ToString(hflEmpCode.Value) == "00631495"
         || Convert.ToString(hflEmpCode.Value) == "00630942" || Convert.ToString(hflEmpCode.Value) == "00631228" || Convert.ToString(hflEmpCode.Value) == "00631357" || Convert.ToString(hflEmpCode.Value) == "00631385" || Convert.ToString(hflEmpCode.Value) == "00631609" || Convert.ToString(hflEmpCode.Value) == "00631441" || Convert.ToString(hflEmpCode.Value) == "00631013" || Convert.ToString(hflEmpCode.Value) == "I-043")
            {
                isCEO = true;
                isHOD = true;
                isMGR = false;
    ...
    private void Reports()
    {
        ...
        spars[0].Value = "thankyoucard_report_admin";
       ...
       spars[2] = new SqlParameter("@card_id", SqlDbType.Int);
        ...
        dsApprovalStatusReport = spm.getDatasetList(spars, "SP_thank_you_card");
        ...
        ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/procs/RptThankyouCard.rdlc");

        ReportDataSource rdsFuel = new ReportDataSource("dsThankyouCard_Count", dsApprovalStatusReport.Tables[0]);
        ReportViewer1.LocalReport.DataSources.Add(rdsFuel);


        ReportDataSource rdsFuel_detail = new ReportDataSource("dsThankyouCard_Send", dsApprovalStatusReport.Tables[1]);
    ...
    private string ParseDate(string input, bool isEndDate = false, string format = "yyyy-MM-dd")
    {
        if (string.IsNullOrWhiteSpace(input))
            return isEndDate ? DateTime.Today.ToString(format) : null;

        string[] parts = input.Trim().Split('/');
        return parts.Length == 3
            ? new DateTime(int.Parse(parts[2]), int.Parse(parts[1]), int.Parse(parts[0])).ToString(format)
            : null;
    }
    ...
    private void getemployee_ReimbursmentDetails()
    {
        try
        {
            ...
            //spars[0] = new SqlParameter("@qtype", SqlDbType.VarChar);
            //spars[0].Value = "thankyoucard_report";


            //dsempreimburstment = spm.getDatasetList(spars, "SP_thank_you_card");
            ...
            //    ReportDataSource rdsFuel = new ReportDataSource("dsThankyouCard_Count", dsempreimburstment.Tables[0]);
            //    ReportDataSource rdsFuel_detail = new ReportDataSource("dsThankyouCard_details", dsempreimburstment.Tables[1]);

            //    ReportViewer2.ProcessingMode = ProcessingMode.Local;
            //    ReportViewer2.LocalReport.ReportPath = Server.MapPath("~/procs/thankyoucard_report.rdlc");
    ...
    public void loadDropDownthankyoucard(string selectVals)
    {
        ...
        spars[0] = new SqlParameter("@qtype", SqlDbType.VarChar);
        spars[0].Value = "thankyoucard";

        dtleaveInboxs = spm.getDropdownList(spars, "SP_thank_you_card");
        if (dtleaveInboxs.Rows.Count > 0)
        {
            ddlthankyoucard.DataSource = dtleaveInboxs;
            //ddlDepartment.DataBind();
            ddlthankyoucard.DataTextField = "card_sub";
            ddlthankyoucard.DataValueField = "id";
            ddlthankyoucard.DataBind();
            ddlthankyoucard.Items.Insert(0, new ListItem("Select Thank You Card", "0")); //updated code

            DataRow[] dr = dtleaveInboxs.Select("card_sub = '" + selectVals.ToString() + "'");
            if (dr.Length > 0)
            {
    ...
    public void loadDropDownemployeename(string selectVal)
    {
        ...
        spars[0] = new SqlParameter("@qtype", SqlDbType.VarChar);
        spars[0].Value = "Emp_Name";

        dtleaveInbox = spm.getDropdownList(spars, "SP_thank_you_card");
