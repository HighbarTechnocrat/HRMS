...
public partial class ABAP_Object_Tracker_HBT_Test_Upload_Test_Cases : System.Web.UI.Page
{
    ...
    private void getProjectLocation()
    {
        try
        {
            ...
            {
                DDLProjectLocation.Items.Insert(0, new ListItem("Select Project Location", "0"));
                lbl_error.Text = "The upload test cases is restricted to HBT Users only.";
            }
        }
        catch (Exception ex)
        {
    ...
    public void GetHBTTestingSignOffDetailsByLocation()
    {
        ...
        spars[0] = new SqlParameter("@qtype", SqlDbType.VarChar);
        spars[0].Value = "GetHBTTesCaseUploadDetailsByLocation";
    ...
    protected void UploadHBTTestCases_btnSave_Click(object sender, EventArgs e)
    {
        ...
        else if (uplHBTTestCases.HasFile)
        {
            ...
            string HBTTestCaseFilePath = Server.MapPath(Convert.ToString(ConfigurationManager.AppSettings["ABAPObjectHBTTest"]).Trim() + "/");
            ...
            filename = "HBTTestCase" + str + InputFile;
        ...
        var objdata = UploadHBTTestCases("UploadHBTTestCases", HBTTestDetailsId, filename);
        ...
        Response.Redirect("~/procs/ABAP_Object_Tracker_HBT_Test_Upload_Test_Cases.aspx");
    ...
    protected void lnkEdit_Click(object sender, ImageClickEventArgs e)
    {
        ...
        txtModule.Text = HttpUtility.HtmlDecode(row.Cells[2].Text);
        txtInterface.Text = HttpUtility.HtmlDecode(row.Cells[4].Text);
        txtFCategory.Text = HttpUtility.HtmlDecode(row.Cells[5].Text);
        txtPriorityOrder.Text = HttpUtility.HtmlDecode(row.Cells[6].Text);
        txtPriority.Text = HttpUtility.HtmlDecode(row.Cells[7].Text);
        txtComplexity.Text = HttpUtility.HtmlDecode(row.Cells[8].Text);
        txtPlannedStart.Text = HttpUtility.HtmlDecode(row.Cells[9].Text);
        txtPlannedFinish.Text = HttpUtility.HtmlDecode(row.Cells[10].Text);
        txtRevisedStart.Text = HttpUtility.HtmlDecode(row.Cells[11].Text.Trim());
        txtRevisedFinish.Text = HttpUtility.HtmlDecode(row.Cells[12].Text);
        txtActualStart.Text = HttpUtility.HtmlDecode(row.Cells[13].Text);
        txtActualFinish.Text = HttpUtility.HtmlDecode(row.Cells[14].Text);
        txtRemark.Text = HttpUtility.HtmlDecode(row.Cells[15].Text);
        txtHBTStatus.Text = HttpUtility.HtmlDecode(row.Cells[16].Text);
        if (row.Cells[16].Text == "Test Case Approved") 
        {
