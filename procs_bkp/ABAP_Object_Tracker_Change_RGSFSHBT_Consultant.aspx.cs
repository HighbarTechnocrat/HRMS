...
public partial class ABAP_Object_Tracker_Change_RGSFSHBT_Consultant : System.Web.UI.Page
{
    ...
    public void DDLProjectLocation_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            if (DS != null && DS.Tables.Count > 0 && DS.Tables[0].Rows.Count > 0)
            {
                ...
                DataTable RGSfilteredTable = originalTable.AsEnumerable()
                                           .Where(row => !row.IsNull("RGSDetailsId"))
                                           .CopyToDataTable();

                DataTable FSfilteredTable = originalTable.AsEnumerable()
                                           .Where(row => !row.IsNull("FSDetailsId"))
                                           .CopyToDataTable();

                DataTable HBTfilteredTable = originalTable.AsEnumerable()
                                           .Where(row => !row.IsNull("HBTDetailsId"))
                                           .CopyToDataTable();
    ...
    protected void updatefs_btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (Convert.ToString(DDLABAPDevDesc.SelectedValue).Trim() == "0" || Convert.ToString(DDLABAPDevDesc.SelectedValue).Trim() == "")
        {
            lbl_FSConsultant_Error.Text = "Please select at least one development description to changes the consultant.";
            return;
        ...
        foreach (GridViewRow row in gvRGSDetails.Rows)
        {
            ...
            var objdata = UpdateConsultantDB("ChangeRGSFSHBTCTMConsultant", RGSDetailsId, FunctionalConsultant, "RGS", txtRemarks.Text);
            if (objdata != null || objdata.Tables.Count > 0 || objdata.Tables[0].Rows.Count > 0)
            {
                if (objdata.Tables[0].Rows[0]["ResponseMsg"].ToString() == "Records udpate sucessfully.")
                {
                    ...
                    #region Send Email 
                    string strSubject = "";
                    strSubject = DDLProjectLocation.SelectedItem.Text.ToString().Trim() + " ABAP Object " + DDLABAPDevDesc.SelectedItem.Text.ToString().Trim();
                    ...
                    strbuild.Append("<p> This is to inform you that you have assigned to new object <b>" + DDLABAPDevDesc.SelectedItem.Text.ToString().Trim() + "</b>. Please co-ordinatinate with <b>" + hdnOldRGSConsultantName.Value.ToString().Trim() + "</b>.</p>");
        ...
        foreach (GridViewRow row in gvFSDetails.Rows)
        {
            ...
            if (objdata != null || objdata.Tables.Count > 0 || objdata.Tables[0].Rows.Count > 0)
            {
                if (objdata.Tables[0].Rows[0]["ResponseMsg"].ToString() == "Records udpate sucessfully.")
                {
                    ...
                    strbuild.Append("<p> This is to inform you that you have assigned to new object <b>" + DDLABAPDevDesc.SelectedItem.Text.ToString().Trim() + "</b>. Please co-ordinatinate with <b>" + hdnOldFSConsultantName.Value.ToString().Trim() + "</b>.</p>");
        ...
        foreach (GridViewRow row in gvHBTTestDetails.Rows)
        {
            ...
            if (objdata != null || objdata.Tables.Count > 0 || objdata.Tables[0].Rows.Count > 0)
            {
                if (objdata.Tables[0].Rows[0]["ResponseMsg"].ToString() == "Records udpate sucessfully.")
                {
                    ...
                    strbuild.Append("<p> This is to inform you that you have assigned to new object <b>" + DDLABAPDevDesc.SelectedItem.Text.ToString().Trim() + "</b>. Please co-ordinatinate with <b>" + hdnOldHBTConsultantName.Value.ToString().Trim() + "</b>.</p>");
