...
public partial class procs_VSCB_Invoice_SecurityDepositAmount : System.Web.UI.Page
{
	...
	{
		DTPoWoNumber = spm.dtPOWoCreate("POWO_List_forSecurityDeposit_Invoice", Convert.ToString(hdnEmpCpde.Value));
	...
	private void PoWODetails(int POID)
	{
		try
		{
           ...
           /* if (DTPoWoNumber.Tables[2].Rows.Count > 0)
            ...
            #region check GST Applicable for Selected PO
            ...
            #region get Security Deposit Balance Amount
            hdn_SecurityDeposit_Balance_Amt.Value = "0";
            if (DTPoWoNumber.Tables[5].Rows.Count > 0)
            {
                if (Convert.ToString(DTPoWoNumber.Tables[5].Rows[0]["Security_Deposit_Amt_Paid"]).Trim() != "")
                { 
                    hdn_SecurityDeposit_Balance_Amt.Value=Convert.ToString(Convert.ToDouble(DTPoWoNumber.Tables[5].Rows[0]["Security_DepositAmt"]) - Convert.ToDouble(DTPoWoNumber.Tables[5].Rows[0]["Security_Deposit_Amt_Paid"])).Trim();
                }
            }
            #endregion
    ...
    public void Check_Advance_Payment_POWO()
    {
        ...
        spars12[1].Value = Convert.ToString(hdnEmpCpde.Value);
        spars12[2] = new SqlParameter("@POID", SqlDbType.VarChar);
        if (Convert.ToString(lstPOWONumber.SelectedValue).Trim() != "")
            spars12[2].Value = Convert.ToDouble(lstPOWONumber.SelectedValue);
        else
    ...
    private void CalculateGSTAmount()
    {
        ...
        if (Convert.ToString(hdn_SecurityDeposit_Balance_Amt.Value).Trim() != "")
        {
            double dTotalProcessInvoiceAmt = 0;

            dTotalProcessInvoiceAmt = Convert.ToDouble(hdn_SecurityDeposit_Balance_Amt.Value);

            if (dAdvAmount > dTotalProcessInvoiceAmt)
            {
                txtAmountPaidWithoutTax.Text = "";
                lblmessage.Text = "Security Deposit Amount cannot be exceed balance Balance Amount (" + Convert.ToString(hdn_SecurityDeposit_Balance_Amt.Value).Trim() + ")   ";
                return;
            }
