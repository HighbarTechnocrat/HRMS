...
public partial class VSCB_CreateInvoice : System.Web.UI.Page
{
    ...
    private void Page_Load(object sender, System.EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                lblheading.Text = "Create Invoice against Security Deposit Balance Amount";
    ...
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        ...
        //cancelled Invoice
        spm.Cancel_Invoicedetails(Convert.ToString(txtEmpCode.Text).Trim(), Convert.ToDouble(hdnInvoiceId.Value), Convert.ToString(txtInvocieRemarks.Text));
        ...
        dsMilestone = spm.get_Invoice_Approver_List(Convert.ToDouble(hdnInvoiceId.Value), txtEmpCode.Text, hdnProject_Dept_Name.Value, Convert.ToString(sPotypeid));
    ...
    private void check_get_GSTPercenatgeAmt()
    {
        ...
        decimal dTotalInvoiceAmt = Convert.ToDecimal(txtAmtWithTax_Invoice.Text);

        if (Convert.ToString(txt_Pay_Adv_Bal_Amt.Text).Trim() != "")
            dMilestoneBalAmt = Convert.ToDecimal(txt_Pay_Adv_Bal_Amt.Text);
        ...
        if (chkIsInvoiceWithoutPO.Checked == false)
        {
            if (Convert.ToString(txtAmtWithTax_Invoice.Text).Trim() != "")
            {
                ...
                {
                    //txtAmtWithTax_Invoice.Text = "";
                    //lblmessage.Text = "Overall Invoice amount Exceeding than Security Deposit Balance Amount. Please correct and try again!";
                    //return;
                }
            }
        }
        ...
        spars4[0].Value = "POWO_SecurityDeposit_BalanceAmt";
        ...
        {
            if (Convert.ToString(dtBal.Rows[0]["Security_BalanceAmt"].ToString()) != "")
            {
                AdvBalAmt = Convert.ToDecimal(dtBal.Rows[0]["Security_BalanceAmt"].ToString());
            }

            if (Convert.ToDecimal(txtAmtWithTax_Invoice.Text) > AdvBalAmt)
            {
                //txtAmtWithTax_Invoice.Text = "";
                //lblmessage.Text = "Overall Invoice amount Exceeding than Security Deposit Balance Amount. Please correct and try again!";
                //return;
    ...
    public void GetPODetails_List()
    {
        ...
        spars[0].Value = "POWO_List_forSecurityDeposit_Invoice"; // "get_Approved_POWOList_CreateInvoice";
