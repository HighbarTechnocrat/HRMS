...
public partial class View_Appreciation_Letter : System.Web.UI.Page
{
    ...
    private void InboxLeaveReqstList()
    {
        try
        {
            ...
            SqlParameter[] spars = new SqlParameter[2];
            spars[0] = new SqlParameter("@qtype", SqlDbType.VarChar);
            spars[0].Value = "GetAppreciationLetter_list";
            ...
            if (dtleaveInbox.Tables[0].Rows.Count > 0)
            {
    ...
    protected void lnkLeaveDetails_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (hdnReqid.Value != "0")
            {
                Response.Redirect("Appreciation_Letter_Details.aspx?reqid=" + hdnReqid.Value);
            }
