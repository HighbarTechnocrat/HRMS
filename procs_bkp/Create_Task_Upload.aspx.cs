...
public partial class Create_Task_Upload : System.Web.UI.Page
{
    ...
    protected void tsktemplate_btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (uplTaskTemplate.HasFile)
        {
            ...
            using (XLWorkbook workBook = new XLWorkbook(read))
            {
                ...
                foreach (IXLRow row in workSheet.Rows())
                {
                    ...
                    if (firstRow)
                    {
                        ...
                        {
                            dtTaskTemp.Columns.Add(cell.Value.ToString());
                        }
                        firstRow = false;
                    }
                    else
                    {
                        ...
                        foreach (IXLCell cell in row.Cells())
                        {
                            ...
                            {
                                lblmessage.Text = "One or more columns contain empty values";
                                return;
                            }
                            else
                            {
            ...
            if (dtTaskTemp.Rows.Count > 0)
            {
                ...
                {
                    if (row[columnIndexToCheck] == DBNull.Value || string.IsNullOrWhiteSpace(row[columnIndexToCheck].ToString()))
                    {
                        hasEmptyValues = true;
                        break; // Exit the loop if any empty value is found
                    }
                    #region save uploaded template data as draft
    ...
    protected int SaveTaskAsDraft(int inputParam, DataRow dr)
    {
        ...
        var getRefId = Convert.ToDouble(hdnTaskRefID.Value);
        var getOldTaskRef = Convert.ToDouble(hdnOldTaskRefID.Value);
        var getMettingDiscussionTitle = dr["MeetingDiscussionTitle"].ToString();
        var getMettingDiscussionDate = DateTime.ParseExact(dr["MeetingDiscussionDate"].ToString(), "dd.MM.yyyy", CultureInfo.InvariantCulture).ToString("yyyy-MM-dd");

        var GetData = GetIdsBasedOnColumns("GetIdsBasedOnColumns", dr["MeetingDiscussionType"].ToString().Trim(), dr["TaskSupervisor"].ToString().Trim(), dr["TaskExecutor"].ToString().Trim(), dr["ProjectLocation"].ToString().Trim());
        var getTaskExecutor = "";
        ...
        var getTaskDescripation = dr["TaskDescription"].ToString();
        var getTaskRemark = dr["TaskRemarks"].ToString();
        var getDueDate = DateTime.ParseExact(dr["TaskDueDate"].ToString(), "dd.MM.yyyy", CultureInfo.InvariantCulture).ToString("yyyy-MM-dd");
        var FinalDueDate = "";
        var getReminder_Due_Date = false;
        var getReminderDay = 0;
        var getEscalationDay = 0;
        ...
        var getStatus = spm.InsertUpdateTaskDetails(0, 0, "InsertUploadedTaskRef", getCreatedBy, getCreatedBy, getMettingDiscussionTitle, getMettingDiscussionDate, Convert.ToInt32(getMeetingDiscussionType), false, getOldTaskRef, "", "", "", "", "", "", false, 0, false, 0, false, 0, "", inputParam);
        hdnTaskRefID.Value = getStatus.ToString();
