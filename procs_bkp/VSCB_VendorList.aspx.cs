...
public partial class procs_VSCB_VendorList : System.Web.UI.Page
{
    ...
    private void getMngTravelReqstList()
    {
        try
        {
            ...
            spars[0].Value = "get_msgVendorList";

            spars[1] = new SqlParameter("@empcode", SqlDbType.VarChar);
            spars[1].Value = strempcode;

            spars[2] = new SqlParameter("@VendorId", SqlDbType.BigInt);
            if (Convert.ToString(lstVendorCode.SelectedValue).Trim() != "0")
                spars[2].Value = Convert.ToDouble(lstVendorCode.SelectedValue); 
            else
                spars[2].Value = DBNull.Value;

            spars[3] = new SqlParameter("@spaymentId", SqlDbType.VarChar);
            if (Convert.ToString(lstVendorName.SelectedValue).Trim() != "0")
                spars[3].Value = lstVendorName.SelectedItem.Text.Trim();
            else
            ...
            else
            {
                ...
                {
                    RecordCountDelete = dsmyInvoice.Tables[2].Rows.Count;
                    RecordCount.Text = "Record Count : " + Convert.ToString(dsmyInvoice.Tables[2].Rows.Count);
                    gvMngTravelRqstList.DataSource = dsmyInvoice.Tables[2];
                    gvMngTravelRqstList.DataBind();
                }
                else
                {
