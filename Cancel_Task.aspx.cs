...
public partial class Cancel_Task : System.Web.UI.Page
{
            ...
            foreach (GridViewRow row in dg_OldTaskDetails.Rows)
            {
                ...
                if (ddl1.Checked == true)
                {
                    ...
                    if (getRefAdd == 0)
                    {
                        ...
                        var getStatus = spm.InsertUpdateTaskDetails(0, 0, "AddTaskReference", empCode, empCode, getMettingDiscussionTitle, getMettingDiscussionDate, Convert.ToInt32(getMettingDiscussionType), false, getOldTaskRef, "", "", "", "", "", "", false, 0, false, 0, false,0);
                    ...
                    var getStatusTask = spm.InsertUpdateTaskDetails(getlblTask_id, getRefAdd, "AddTaskRef", empCode, empCode, "", "", 0, false, getOldTaskRef, "", "", "", "", "", "", false, 0, false, 0, false,0);
    ...
    protected void lnk_Final_Submit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            var getMettingDiscussionType = Convert.ToString(ddl_Meeting_Type.SelectedValue).ToString();
            var getCreatedBy = Convert.ToString(Session["Empcode"]).Trim();
            var getStatus = spm.InsertUpdateTaskDetails(Convert.ToInt32(getRefId), getRefId, "UpdateTaskRef", getCreatedBy, getCreatedBy, getMettingDiscussionTitle, getMettingDiscussionDate, Convert.ToInt32(getMettingDiscussionType), false, getOldTaskRef, "", "", "", "", "", "", false, 0, false, 0, false,0);
            //
                    ...
                    if (getTable != null)
                    {
                        if (getTable.Rows.Count > 0)
                        {
                            foreach (DataRow item in getTable.Rows)
                            {
                                ...
                                var getStatus1 = spm.InsertUpdateTaskDetails(id, getTaskRefId, "UpdateNewTaskDetails", getCreatedBy, getCreatedBy, getMettingDiscussionTitle, getMettingDiscussionDate, Convert.ToInt32(getMettingDiscussionType), getFor_Info, getOldTaskRef, getTaskSupervisor, getTaskExecutor, getTaskDescripation, getTaskRemark, getDueDate, getTask_ID, getReminder_Due_Date, getReminderDay, getEscalation_Due_Date, getEscalationDay, getEscalation_Repeate, tempTaskRefId);
